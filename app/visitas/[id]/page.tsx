...
interface Labor { id: number; tipo: string; descripcion: string; estado: string; responsable: Usuario }
interface Aplicacion { id: number; producto: string; tipoProducto: string; dosis: number; unidad: string; estado: string; tecnico: Usuario }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            {visita.estado === 'EN_PROGRESO' && (
              <button onClick={() => setModal('labor')} className="btn-primary text-xs py-1.5 px-3">+ Agregar labor</button>
            )}
          </div>
          <div className="space-y-2">
            ...
            {visita.labores.map((l) => (
              <div key={l.id} className="card py-3 px-4">
                ...
                <p className="text-xs text-gray-400 mt-0.5">{l.tipo} · {l.responsable.nombre} {l.responsable.apellido}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            ...
            {visita.aplicaciones.map((a) => (
              <div key={a.id} className="card py-3 px-4">
                ...
                <p className="text-xs text-gray-400 mt-0.5">{a.dosis} {a.unidad}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 p-4 space-y-2">
              {laboresDisponibles.length === 0 && (
                <p className="text-gray-400 text-sm text-center py-4">No hay labores en el catálogo para {visita.predio.cultivo}</p>
              )}
              {laboresDisponibles.map((item) => (
                <div
                  key={item.id}
