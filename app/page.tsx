  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <StatCard title="Predios Activos" value={prediosActivos} subtitle="en producción" color="green"
        ...
        />
        <StatCard title="Labores Activas" value={laboresActivas} subtitle="pendientes o en progreso" color="blue"
          icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>}
        />
        <StatCard title="Aplicaciones del Mes" value={aplicacionesMes} subtitle="este mes" color="purple"
          icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>}
        />
        <StatCard title="Equipo Técnico" value={totalTecnicos} subtitle="miembros activos" color="orange"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/labores" className="text-primary-600 hover:text-primary-700 text-xs font-medium">Ver todas →</a>
          </div>
          <div className="space-y-3">
            {ultimasLabores.length === 0 && <p className="text-gray-400 text-sm text-center py-4">Sin labores registradas</p>}
            {ultimasLabores.map((labor) => (
              <div key={labor.id} className="flex items-start justify-between py-2 border-b border-gray-100 last:border-0">
                ...
                <div className="ml-3 flex flex-col items-end gap-1">
                  <span className={`badge ${estadoLaborColor(labor.estado)}`}>{labor.estado.replace('_', ' ')}</span>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/aplicaciones" className="text-primary-600 hover:text-primary-700 text-xs font-medium">Ver todas →</a>
          </div>
          <div className="space-y-3">
            {ultimasAplicaciones.length === 0 && <p className="text-gray-400 text-sm text-center py-4">Sin aplicaciones registradas</p>}
            {ultimasAplicaciones.map((ap) => (
              <div key={ap.id} className="flex items-start justify-between py-2 border-b border-gray-100 last:border-0">
                ...
                <div className="ml-3 flex flex-col items-end gap-1">
                  <span className={`badge ${tipoProductoColor(ap.tipoProducto)}`}>{labelFromValue(TIPOS_PRODUCTO, ap.tipoProducto)}</span>
