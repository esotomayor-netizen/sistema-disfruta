...
export default function EditarPredioPage() {
  ...
  useEffect(() => {
    fetch('/api/predios')
      .then((r) => r.json())
      .then((predios: { id: number; nombre: string; superficie: number; cultivo: string; ubicacion: string; activa: boolean }[]) => {
        const p = predios.find((x) => x.id === parseInt(id as string))
  ...
  return (
    <div>
      <Header title="Editar Predio" subtitle="Actualizar información del predio" action={<Link href="/predios" className="btn-secondary">← Volver</Link>} />
