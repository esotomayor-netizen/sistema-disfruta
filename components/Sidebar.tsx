    ...
    <aside className="w-64 min-h-screen bg-primary-900 flex flex-col">
      <div className="px-6 py-6 border-b border-primary-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-bold text-sm">Sistema Disfruta</p>
            <p className="text-primary-400 text-xs">Gestión Agrícola</p>
          </div>
        </div>
      </div>
        ...
        {nav.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-150 ${active ? 'bg-primary-700 text-white' : 'text-primary-300 hover:bg-primary-800 hover:text-white'}`}
            >
              {item.icon}
              {item.label}
            </Link>
      ...
      <div className="px-6 py-4 border-t border-primary-800">
        <p className="text-primary-500 text-xs">© 2026 Fundo Disfruta</p>
      </div>
    </aside>
  )
}
